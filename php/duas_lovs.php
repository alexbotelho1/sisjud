...
$curs2 = ocinewcursor($conn);
$stmt = ociparse($conn,"begin rh.srec.retorna_duas_lovs(:InUsuario, :InTabela,:InFiltro, :InParametro, :outresultado,:outresultado2); end;");
		ocibindbyname ($stmt,":InUsuario",$_SESSION["v_ids"]);
		ocibindbyname ($stmt,":InTabela",$tabela);
		ocibindbyname ($stmt,":InFiltro",$filtro);
		...
		ocibindbyname ($stmt,":outresultado2",$curs2,-1,OCI_B_CURSOR);
		$r = @ociexecute($stmt);
		if (!$r) {
			erro ($stmt);
		}
		...
		ociexecute($curs2);
		while (ocifetchinto($curs2,$outresultado)){
			//MONTA XML RETORNO
            $_tabela = $_docxml->createElement('seg_tabela');

...
$(document).ready(function()
{
	...
	// Ao Lançar um Certificado, as opções de Lançamentos de Certificado (combolist, Input, button) desaparecem p/ ficar mais fácil a Visualização do Parecer.
		function someCertificado()		
	{ 	$('#table_certificado01').hide();
		$('#table_certificado02').hide();
		$('#tbfoot2').show();
	};
	
	// Faz aparecer as opções de lançamento de um Certificado, opções essas que desapareceram por motivo de estética, conforme função someCertificado().
		function exibeCertificado()		
	{ 	$('#tbfoot2').hide();
		$('#table_certificado01').show();
		$('#table_certificado02').show();
		$('#Bt_InsereCertificado').show();
		//$('#Bt_atualizaCertificado').hide();
		$('#datainicio').focus();		
	};
	...
	function InsereCertificado() {
			...
			{cod_matricula:$('#codigo').val(), cpf:$("#cpf").val(), nome_agraciado:$("#nome_agraciado").val(), nome_al_conv:$("#nome_al_conv").val(), setor_al_conv:$("#setor_al_conv").val(), nome_ins_conv:$("#nome_ins_conv").val(), setor_ins_conv:$("#setor_ins_conv").val(), cod_curso:$("#cod_curso").val(), nome_curso:$("#nome_curso").val(), dt_inicio:$("#dt_inicio").val(), dt_fim:$("#dt_fim").val(),  dt_exp:$("#dt_exp").val(), carga:$("#carga").val(), localidade:$("#localidade").val(), form_prof:$("#form_prof").val() form_acad:$("#form_acad").val()},
		function(a)
		{	if ( !trataErro(a) )	
			{ return false;	}
			alert( $("msg4",a).text() );

...
function trataErro(xmldoc){
        if (xmldoc!=null) {
	        ...
	        if(no!=null){
					  ...
					  var p0=err.indexOf('[')
					  var p1=err.indexOf(']')
					  if (p0>0) { err = err.substr(p0+1,p1-p0-1) }
	                  alert(err);
	                  return false;
	               }
	        else{
...
function preenche_select (nos,div,sel){
...
function carrega_duas_lovs(tabela, div, sel,div2,sel2){
var a = new AJAX();
a.url = "php/duas_lovs.php";
a.metodo = 'POST';
a.modo = 'X';
a.params = 'tabela='+encodeURI(tabela);
a.params = a.params +'&filtro='+encodeURI(sel);
...
a.addHeader("Content-type", "application/x-www-form-urlencoded");
	a.processaresultado = function(xmldoc){
         ...
         var nos = xmldoc.getElementsByTagName('tabela');
		   if(xmldoc.hasChildNodes()&&nos.length>0){
		   preenche_select(nos,div,sel);
		   }
         nos = xmldoc.getElementsByTagName('seg_tabela');
		   if(xmldoc.hasChildNodes()&&nos.length>0){
		   preenche_select(nos,div2,sel2);
		   }		 
 	}
	a.conectar();

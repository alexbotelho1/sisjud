...
$(document).ready(function(){
   ...
   preencheComHash('lista_base_legal','todos','base_legal','...');
	...
	function exibeResultado(xmldoc) {
        ...
        // cabeçalho da tabela
         ...
         if(xmldoc.hasChildNodes()&&nos.length>0){
			...
			{
				d.innerHTML = 'Não existe nenhum Atestado Médico cadastrado';
			};
	...
	// Reimpressão do ùltimo Parecer (em caso de falha da impressão ou na geração do Pdf.)
	function ReimpParecer()   
	{	// Se não preencer os 4 campos abaixos requeridos p/ gerar a reimpressão de parecer: aparece uma mensagem de alerta
		if ($("#tipo").val()=='0') 
			{ 	alert('Falta Preencher o Tipo de Licença');
				return false; }   
		else if ($("#processo").val()=='') 
			{   alert('Falta Preencher o Número do Processo');
				return false; }
		else if ($("#protocolo").val()=='') 
			{   alert('Falta Preencher o Número do Protocolo');
				return false; }
